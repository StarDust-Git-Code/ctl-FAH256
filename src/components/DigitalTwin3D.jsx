...
export default function DigitalTwin3D({ shipment, onTriggerBreach, isDarkMode = true }) {
  ...
  const [simulatedTemp, setSimulatedTemp] = useState(shipment ? shipment.currentTemp : -72.4);
  ...
  useEffect(() => {
    ...
    const isExcursion = simulatedTemp > (shipment ? shipment.maxSafeTemp : 6) || simulatedTemp < (shipment ? shipment.minSafeTemp : -80);
  ...
  const handleReset = () => {
    setSimulatedTemp(shipment ? shipment.currentTemp : -72.4);
    setIsTampered(false);
    handleResetZoom();
  };
      ...
      <div className={`flex items-center justify-between mb-3 border-b pb-3 ${
        ...
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-0.5 rounded-full font-mono font-bold ${
            isTampered || simulatedTemp > (shipment?.maxSafeTemp ?? 6)
      ...
      <div className={`relative w-full h-[320px] rounded-lg overflow-hidden cursor-grab active:cursor-grabbing border ${
        ...
        <div className={`absolute top-3 left-3 backdrop-blur-md border rounded-lg p-3 text-xs font-mono space-y-1 shadow-md ${
          ...
          <div className="text-[11px] text-slate-400">
            Target Window: <span className={isDarkMode ? 'text-slate-200 font-semibold' : 'text-slate-900 font-semibold'}>{shipment?.minSafeTemp ?? -80}°C to {shipment?.maxSafeTemp ?? -60}°C</span>
          </div>
        </div>
        ...
        <div className={`absolute top-3 right-3 backdrop-blur-md border rounded-lg p-2.5 shadow-md ${
          ...
          <div className="grid grid-cols-4 gap-2 text-[9px] font-mono text-center">
            ...
            <div className="flex flex-col items-center">
              <div className={`w-3 h-3 rounded-full mb-0.5 ${isTampered || simulatedTemp > (shipment?.maxSafeTemp ?? 6) ? 'led-error' : 'led-off'}`}></div>
              <span className={isTampered || simulatedTemp > (shipment?.maxSafeTemp ?? 6) ? 'text-red-400 font-bold animate-pulse' : 'text-slate-500'}>ERROR</span>
            </div>
          </div>
        </div>
